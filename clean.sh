...
docker run --entrypoint="" $PGARGS chown -R $MYUID /var/lib/postgresql/data
...
docker run --entrypoint="" $SYNAPSEARGS find /data/ ! -type d -delete
docker run --entrypoint="" $SYNAPSEARGS chown -R $MYUID /data/
